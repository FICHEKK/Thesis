...
\zahvala{Special thanks to my dear mentor doc. dr. sc. Mirko Sužnjević for giving me the opportunity to work on something I am truly passionate about.}
...
\chapter{Conclusion}
This thesis documents and explains the theory, implementation and application of a networking library designed for developing multiplayer games. Thesis consists of three main chapters, each documenting a single layer in the stack.

\begin{enumerate}
	\item \textbf{Transport layer library} describes an implementation of a low-level reliable UDP library. It covers efficient data serialization via packet abstractions, multiple channel implementations which solve ordering, duplication, reliability and fragmentation, virtual connections and client/server implementations.
	\item \textbf{High-level library} explains Unity specific components that create powerful abstractions and allow users to easily develop complex networking solutions. It covers identifying, spawning, toggling and destroying of networked objects, state-synchronization and RPC implementations as well as automatic transform synchronization.
	\item \textbf{Application} showcases a multiplayer first-person shooter developed using high-level library. It covers concepts of creating and joining virtual game sessions, networked enemy waves, pick-ups and overall game logic.
\end{enumerate}
...
\keywords{Networking, multiplayer, videogame, library}
\end{abstract}
...
\kljucnerijeci{Umrežavanje, višekorisnička, videoigra, knjižnica}
\end{sazetak}
