...
Because of its importance, it is also the only singleton\footnote{https://en.wikipedia.org/wiki/Singleton\_pattern} component, so that it can be conveniently accessed from anywhere in the application.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.75]{NetworkObject-example}
...
There are two types of networked objects: those that are a embedded as a part of the Unity scene and those that are dynamically spawned during runtime.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|}
		...
		Prefab ID      & \begin{tabular}[c]{@{}c@{}}Identifier of the prefab from which\\ this networked object has been created.\\ In a case of scene object, it is set to -1.\end{tabular} \\ \hline
		Object ID      & \begin{tabular}[c]{@{}c@{}}Unique identifier of the specific\\ network object instance.\end{tabular}                                                                \\ \hline
		Is Owner       & \begin{tabular}[c]{@{}c@{}}If true, object can be controlled by the\\ local process, otherwise it can't.\end{tabular}                                               \\ \hline
	\end{tabular}
	\caption{NetworkObject fields.}
	\label{table:network-object-fields}
\end{table}
...
\chapter{Application}
This chapter showcases a multiplayer game that was build by using previously described high-level networking library.
